...
namespace boost {
	class column {
		...
		public:		
		column(std::shared_ptr<sheet_interface> obj, std::size_t row) : obj_(obj), row_(row), current_cell_(obj->get_cell_unchecked(row_,0)) {
		}
		...
		boost::cell& set_current_cell(std::size_t column) {
			current_cell_ = obj_->get_cell_unchecked(row_,column);
