...
int basic_chart_test(boost::document& c) {
	try {
			...
			boost::chart chrt = s1.add_chart("Test1", "A1:N4", 100, 3000, 20000, 8000);
			chrt.set_type(boost::chart_type::SCATTER, false);
			chrt.set_axis_title(boost::chart_axis::X, "Ghosh");
			chrt.set_axis_orientation(boost::chart_axis::X, false);
			chrt.set_range("A1:N4");
			chrt.set_position(200, 3000);
